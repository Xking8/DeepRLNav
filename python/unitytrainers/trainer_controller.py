...
class TrainerController(object):
        ...
        if self.curriculum_file is not None:
            ...
            if self.env.curriculum.measure_type == "progress":
                ...
                for brain_name in self.env.external_brain_names:
                    ...
                    progress += self.trainers[brain_name].get_success_rate()
                return progress
            else:
                return None
        else:
            ...
            try:
                while any([t.get_step <= t.get_max_steps for k, t in self.trainers.items()]) or not self.train_model:
                    if self.env.global_done:
                        print("----------------------------------------IN GLOBAL DONE")
                        self.env.curriculum.increment_lesson(self._get_progress())
                        #print("progress:" + str(self._get_progress()))
                        print("----------------------------------------IN GLOBAL DONE2")
                        curr_info = self.env.reset(train_mode=self.fast_simulation)
                        for brain_name, trainer in self.trainers.items():
                            trainer.end_episode()
                    ...
                    '''for brain_name, trainer in self.trainers.items():
                        ...
                        #import scipy.misc
                        #scipy.misc.imsave('file.jpg', curr_info[brain_name].visual_observations[1][0])
                        print (str(curr_info[brain_name].visual_observations[1].shape))
                        #curr_info[brain_name].visual_observations[1] =
    ...
    def subsampling(self, image, size):
        ...
        return image
        #import scipy.misc
        #scipy.misc.imsave('outfile.jpg', image[0])
        '''
        for i in range(int(image.shape[1])):
            for j in range(int(image.shape[1])):
                print(str(i) + "," + str(j) + ": " + str(image[0][i][j][0]),str(image[0][i][j][1]),str(image[0][i][j][2]))
        '''
        #exit(1)